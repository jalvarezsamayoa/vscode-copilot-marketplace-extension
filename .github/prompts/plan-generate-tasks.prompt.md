---
name: plan-create-task-list
description: Generates a detailed task list in Markdown format based on the PRD generated by plan-create-prd.prompt.md.
---
...
1. **Receive Requirements:** Read the PRD generated by the plan-create-prd.prompt.md process from `tasks/[feature-name]/prd.md`.
2. **Analyze Requirements:** The AI analyzes the functional requirements, user needs, and implementation scope from the PRD.
...
8. **Save Task List:** Save the generated document in the `/tasks/[feature-name]` directory with the filename `tasks.md`, where `[feature-name]` describes the main feature or task being implemented (e.g., if the request was about user profile editing, the output is `/tasks/user-profile-editing/tasks.md`).
...
- `path/to/prd.md` - The PRD document that outlines the feature requirements.
...
- `path/to/relevant/*.instructions.md` - Instructions specific to the feature or task, if any.
